...
function updateStatus() {
  ...
  let isOpen = false;
  // Horaires: Mer-Jeu-Dim 17h30–02h, Ven-Sam 17h30–03h, Lun-Mar fermé
  ...
  // After midnight portion: Mer/Jeu/Dim close at 02h (120min), Ven/Sam close at 03h (180min)
  if (openEvening.includes(day) && t >= 1050) {
    isOpen = true; // evening session started
  } else if ([0,3,4].includes(prevDay) && t < 120) {
    isOpen = true; // after midnight, prev day was Mer/Jeu/Dim → open until 02h
  } else if ([5,6].includes(prevDay) && t < 180) {
    isOpen = true; // after midnight, prev day was Ven/Sam → open until 03h
  }
  ...
  if (isOpen) {
    ...
    let closeTime = '';
    if (openEvening.includes(day) && t >= 1050) {
      if ([0,3,4].includes(day)) closeTime = '02h';
      else if ([5,6].includes(day)) closeTime = '03h';
    } else if ([0,3,4].includes(prevDay) && t < 120) { closeTime = '02h';
    } else if ([5,6].includes(prevDay) && t < 180) { closeTime = '03h'; }
